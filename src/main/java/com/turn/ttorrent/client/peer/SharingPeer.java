...
public class SharingPeer extends Peer implements MessageListener {
	 ...
	 * 
	 * @param socket
	 *            The connected socket for this peer.
	 */
	...
	public synchronized void handleMessage(PeerMessage msg) {
		...
		case PORT:
			//dhtPort = ((PortMessage) msg).getPort();		<- original buggy code.
		
			// Experimental code ->
			PortMessage message = null;
			
			try {
				message = (PortMessage) Message.parse(msg.getData(), this.torrent);
				dhtPort = message.getPort();
